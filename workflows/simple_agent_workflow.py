...
class SimpleAgentWorkflow:
    ...
    async def _run_react_loop(
        ...
        workflow.logger.info(
            f"[SimpleAgentWorkflow] Starting React agent loop, max iterations: {max_iterations}"
        )

        # Loop until we get "finish" or hit max iterations
        while current_iteration <= max_iterations:
            workflow.logger.info(
                f"[SimpleAgentWorkflow] Iteration {current_iteration}/{max_iterations}"
            )
            ...
            if agent_result.status != ActivityStatus.SUCCESS:
                workflow.logger.error(
                    f"[SimpleAgentWorkflow] ReactAgent failed: {agent_result.error}"
                )
            ...
            workflow.logger.info(
                f"[SimpleAgentWorkflow] Agent decision - Tool: {tool_name}, Args: {tool_args}"
            )

            # Check if we're done
            if tool_name == "finish":
                workflow.logger.info("[SimpleAgentWorkflow] Agent selected 'finish' - task complete")
        ...
        if current_iteration > max_iterations:
            workflow.logger.warning(
                f"[SimpleAgentWorkflow] Reached maximum iterations ({max_iterations})"
            )
    ...
    async def _call_react_agent(
        ...
        try:
            ...
            workflow.logger.debug(
                f"[SimpleAgentWorkflow] ReactAgent returned trajectory with keys: {list(result.trajectory.keys())}"
            )
            
            return result
        except Exception as e:
            workflow.logger.error(
                f"[SimpleAgentWorkflow] Error calling ReactAgent activity: {e}"
            )
            raise
    ...
    async def _execute_tool(
        ...
        """
        workflow.logger.info(
            f"[SimpleAgentWorkflow] Executing tool: {tool_name} with args: {tool_args}"
        )
        ...
        try:
            ...
            workflow.logger.info(
                f"[SimpleAgentWorkflow] Tool execution completed: {'success' if result.success else 'failed'}"
            )
            return result
        except Exception as e:
            workflow.logger.error(
                f"[SimpleAgentWorkflow] Tool execution error: {e}"
            )
    ...
    async def _extract_final_answer(
        ...
        """
        workflow.logger.info(
            "[SimpleAgentWorkflow] Extracting final answer from trajectory"
        )
        ...
        try:
            ...
            workflow.logger.error(
                f"[SimpleAgentWorkflow] Error calling ExtractAgent activity: {e}"
            )
    ...
    async def run(self, user_message: str, user_name: str = "anonymous") -> Response:
        ...
        # Run the React agent loop for all queries
        ...
        workflow.logger.debug(f"[SimpleAgentWorkflow] ExtractAgent result - Status: {final_answer.status}, Answer: '{final_answer.answer}', Answer type: {type(final_answer.answer)}")
        ...
        if final_answer.status == ActivityStatus.SUCCESS and final_answer.answer is not None and str(final_answer.answer).strip():
            ...
            workflow.logger.info(
                f"[SimpleAgentWorkflow] Successfully extracted answer for {user_name}: {final_answer.answer}"
            )
            ...
            if latest_observation:
                ...
                workflow.logger.info(
                    f"[SimpleAgentWorkflow] Using latest meaningful observation as answer for {user_name}"
                )
            else:
                response_message = "No result found"
                workflow.logger.warning(
                    f"[SimpleAgentWorkflow] No meaningful observations found in trajectory for {user_name}"
                )
        else:
            response_message = f"Error: {final_answer.error or 'Unknown error'}"
            workflow.logger.error(
                f"[SimpleAgentWorkflow] Failed to extract answer for {user_name}: {final_answer.error}"
            )

        # Log summary
        workflow.logger.info(
            f"[SimpleAgentWorkflow] Workflow completed. Tools used: {', '.join(tools_used) if tools_used else 'None'}, "
