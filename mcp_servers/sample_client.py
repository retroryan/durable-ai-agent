    ...
    async with client:
        ...
        try:
            ...
            result = await client.call_tool(
                "get_weather_forecast", {"request": request1}
            )
            ...
            result2 = await client.call_tool(
                "get_weather_forecast", {"request": request2}
            )
        ...
        try:
            ...
            result = await client.call_tool(
                "get_weather_forecast", {"request": london_request}
            )
            if result and hasattr(result[0], "text"):
                data = json.loads(result[0].text)
            ...
            result2 = await client.call_tool(
                "get_weather_forecast", {"request": tokyo_request}
            )
            if result2 and hasattr(result2[0], "text"):
                data2 = json.loads(result2[0].text)
        ...
        try:
            ...
            paris_request = ForecastRequest(
                location="Paris", days=10
            )  # Should be clamped to 7
            result = await client.call_tool(
                "get_weather_forecast", {"request": paris_request}
            )
            if result and hasattr(result[0], "text"):
                data = json.loads(result[0].text)
                print(
                    f"Forecast with days=10 (should be clamped): {json.dumps(data, indent=2)}"
                )
            else:
        ...
        try:
            ...
            coords_request = ForecastRequest(
                latitude=40.7128, longitude=-74.0060, days=2  # NYC coordinates
            )
            result = await client.call_tool(
                "get_weather_forecast", {"request": coords_request}
            )
            if result and hasattr(result[0], "text"):
                data = json.loads(result[0].text)
                print(
                    f"Weather by coordinates: {data['location_info']['name']} ({data['location_info']['coordinates']['latitude']}, {data['location_info']['coordinates']['longitude']})"
                )
...
async def test_with_callbacks():
    ...
    async with client:
        ...
        result = await client.call_tool(
            "get_weather_forecast", {"request": seattle_request}
        )
        if result and hasattr(result[0], "text"):
            data = json.loads(result[0].text)
...
async def test_error_handling():
    ...
    async with client:
        ...
        try:
            result = await client.call_tool(
                "get_weather_forecast", {"invalid_param": "test"}
            )
        except Exception as e:
