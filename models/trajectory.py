...
class Trajectory(BaseModel):
    ...
    @staticmethod
    def get_tools_used_from_trajectories(trajectories: List["Trajectory"]) -> List[str]:
        """Extract unique tools used from trajectories, excluding 'error' and 'finish'."""
        tools_used = []
        for trajectory in trajectories:
            if (trajectory.tool_name and
                trajectory.tool_name not in ['error', 'finish'] and
                trajectory.tool_name not in tools_used):
                tools_used.append(trajectory.tool_name)
        return tools_used
